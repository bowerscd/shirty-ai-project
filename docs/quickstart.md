...
# wrote /etc/huginn/identity.key
# pubkey:      9d2f...4b7c
# fingerprint: 1234abcd5678...
```

Copy the **huginn** pubkey (`9d2f...4b7c`) — you'll paste it into the next
...
huginn identity. It is not a secret — leak it and the worst that happens
...
```bash
scp /tmp/huginn.token home.example.lan:/tmp/
```
...
# updated /etc/huginn/config.toml
...
```bash
sudo rm /tmp/huginn.token
```

## 5. Add a forwarding rule (VPS)

Drop a rule file into `rules_dir`. Each `*.toml` file there can hold any
...
Just drop more `*.toml` files into `/etc/yggdrasil/conf.d/`. No restart
...
`yggdrasilctl rules list` will show all three rules within ~250ms.

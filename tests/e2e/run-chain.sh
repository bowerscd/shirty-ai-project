...
# parallel — 7 concurrent cold `cargo build --release` runs that thrash a
# CI runner and blow the 25-min job timeout (the chain job never reached a
# test phase). See finding `e2e-chain-redundant-parallel-builds`.
"${DC[@]}" "${COMPOSE_ARGS[@]}" build init-chain
